...
class MSEWOA():
    def __init__(self, fit_func, num_dim=30, num_particle=20, max_iter=500, 
        ...
        self.gBest_score = self.score.min().copy()
        self.gBest_X = self.X[self.score.argmin()].copy()
    ...
    def opt(self):
        ...
        while(self._iter<self.max_iter):
            mutation = self.levy_flight(self.X[self.score.argmin(), :])
            if self.fit_func(mutation)<self.gBest_score:
                self.gBest_X = mutation.copy()
                self.gBest_score = self.fit_func(mutation)
                self.gBest_curve[self._iter] = self.gBest_score.copy()
                self.X[self.score.argmin(), :] = mutation.copy()
            elif self.fit_func(mutation)<self.score.max():
                self.X[self.score.argmax(), :] = mutation.copy()
            ...
            if np.min(self.score) < self.gBest_score:
                self.gBest_X = self.X[self.score.argmin()].copy()
                self.gBest_score = self.score.min().copy()
            self.gBest_curve[self._iter] = self.gBest_score.copy()
    ...
    def levy_flight(self, worst_agent):
        num = 1
        idx = np.random.randint(len(worst_agent), size=num)
        beta = 1.5
        sigma_v = 1
        sigma_u = ( ( math.gamma(1+beta)*np.sin(np.pi*beta/2) ) / ( beta*math.gamma((1+beta)/2)*2**((beta-1)/2) ) )**(1/beta)
        
        u = np.random.normal(0, sigma_u**2, size=num)
        v = np.random.normal(0, sigma_v**2, size=num)
        step = u/(np.abs(v)**(1/beta))
        
        new_agent = worst_agent.copy()
        new_agent[idx] = worst_agent[idx] + step
